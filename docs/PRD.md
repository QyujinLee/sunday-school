...
관련 운영 정책 문서: `docs/PROJECT_POLICY.md`
...
- Google 로그인을 완료한 사용자만 실제 데이터에 접근이 가능하다.

### 게스트 둘러보기
- 로그인하지 않아도 `/guest` 이하 경로에서 시스템을 둘러볼 수 있다.
- 실제 데이터를 가리는 방식이 아니라 **데모 데이터**를 보여준다. DB를 조회하지 않는다.
- 쓰기 기능과 외부 링크 메뉴는 제공하지 않는다.
- 진행 상황: 대시보드만 구현됨. 나머지 화면과 로그인 페이지 진입점은 이후 작업(`docs/FEATURE_PLAN.md` 참고).
...
- 예외로 게스트 둘러보기 경로(`/guest` 이하)는 로그인 없이 접근할 수 있다. 데모 데이터만 노출한다.
