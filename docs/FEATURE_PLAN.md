...
**최종 갱신**: 2026-09-14 (기술 검증 및 계획 수립, 구현 전)
...
|---|---|---|---|
| 구글 캘린더 쓰기 (사회·단상 순서, 주간 일정 입력) | **가능** | 서비스 계정 + 캘린더 공유(`writer`), 캘린더를 원본으로 유지 | 서비스 계정 생성·캘린더 공유 후 **실제 쓰기 시험(4-0)** 통과 |
| 게스트 모드 (비로그인 둘러보기) | **가능** | `/guest/*` 별도 경로 + **데모 데이터** | 보여줄 데이터 방식(데모 vs 실데이터 마스킹) 결정 |
...
→ **A 권장.** 1인 운영·무료 운영 제약에 맞고, B는 토큰 만료와 앱 검증 때문에 운영 부담이 크다.
...
→ **A 권장.** 기존 코드가 이미 캘린더를 원본으로 읽고 있어 가장 짧은 경로다. 입력 이력이 필요해지면 B로 확장한다.
...
- **4-2 API route**: 주간 일정 저장 `route.ts`
  - 입력 검증: `src/lib/validation/`에 zod 스키마 추가 + 테스트
  - 권한 가드: `src/lib/api-session.ts` 재사용 (범위는 아래 결정 사항)
  - 성공 시 기존 새로고침과 동일하게 `revalidatePath('/dashboard')`, `revalidatePath('/attendance')` + 인메모리 스냅샷 갱신
- **4-3 UI**: 대시보드 주간 일정 카드에 "수정" 버튼 → 클라이언트 폼(이번 주/다음 주), 클라이언트 컴포넌트 + API route 원칙(`CLAUDE.md` 아키텍처 원칙)
...
→ **A 권장.** 목적이 "시스템이 어떤지 보여주기"라 실데이터가 필요 없다. 부수 효과로 `docs/IMPROVEMENT_PLAN.md` Phase 3(README 스크린샷)을 **개인정보 없이** 게스트 화면으로 찍을 수 있다.
...
1. **캘린더 입력 권한**: 관리자만 / 승인된 교사 전체 (제안: 처음엔 관리자만, 필요 시 확대)
2. **"주간 활동·일정" 범위**: 주간 이벤트 설명의 `[주간 일정]` 목록만 / 날짜별 개별 일정 생성까지
3. **캘린더 데이터 원본**: 캘린더 원본(권장) / DB + 동기화
4. **게스트 데이터**: 데모 데이터(권장) / 실데이터 마스킹
5. **게스트 공개 화면 범위**: 위 제안대로 / 조정
6. **Google Cloud 콘솔 작업**: 서비스 계정 생성·캘린더 공유는 형님 계정 권한이 필요
